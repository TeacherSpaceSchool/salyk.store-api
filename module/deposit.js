const Deposit = require('../models/depositHistory');
...
module.exports.getIntegrationDeposits = async ({legalObject, skip, date, branch, cashbox, cashier, workShift}) => {
    ...
    let _res = await Deposit.find({
        ...
        .limit(100)
        .sort('-createdAt')
        .select('_id number createdAt comment amount branch cashbox cashier workShift')
        .lean()
...
module.exports.getIntegrationDeposit = async ({UUID, legalObject}) => {
    let integrationObject
    let res = await Deposit.findOne({
        legalObject,
        _id: UUID
    })
...
module.exports.putIntegrationDeposit = async ({legalObject, comment, amount, workShift}) => {
    ...
    if(workShift&&cashbox&&((new Date()-workShift.start)/1000/60/60)<24&&amount>0) {
        let number = (await Deposit.countDocuments({cashbox: cashbox._id}).lean())+1;
        let object = new Deposit({
            number,
        ...
        await Deposit.create(object)
        amount = checkFloat(amount)
        workShift.deposit = checkFloat(workShift.deposit + amount)
